...
- **Kein Bauschritt, kein Framework, keine Abhängigkeit im Browser außer `supabase-js` v2 aus dem CDN.** Die Seite muss sich mit `python3 -m http.server` öffnen lassen. Das ist eine didaktische Entscheidung: Studierende sollen jede Zeile lesen können, ohne ein Bündelwerkzeug zu verstehen.
  ...
  > **NACHTRAG 2026-08-27 — diese Regel ist für die Landkarte der Stationen ÜBERHOLT.** Der Auftraggeber, wörtlich, zur ersten (selbst gezeichneten SVG-)Fassung: *„warum verwendest du keine Karte wie Leaflet und machst die Karte selber? Das macht null Sinn."* Er hat recht: eine schematische Fläche ohne Straßen beantwortet „wo ist diese Station" gerade nicht — hier ist die didaktische Begründung oben (jede Zeile lesbar, kein Bündelwerkzeug) gegen eine andere didaktische Begründung abgewogen und unterlegen: eine Karte, die die eigentliche Frage nicht beantwortet, lehrt auch keinen sauberen Datenbankentwurf besser vor.
  >
  > **Was das konkret ändert:** `wawi/index.html` bindet zusätzlich Leaflet 1.9.4 aus demselben CDN-Muster wie `supabase-js` (`<script>`/`<link>`, Version fest verankert, kein `@latest`) sowie Kartenkacheln von `tile.openstreetmap.org` zur Laufzeit. Die Kacheln sind dabei etwas ANDERES als `supabase-js`: `supabase-js` liefert nur JAVASCRIPT-CODE aus, den der Browser einmal lädt — die Kartenkacheln sind eine FORTLAUFENDE Fremdanfrage bei jedem Kartenblick, die dem Kachelserver IP-Adresse und Kartenausschnitt mitteilt. Die ausführliche Datenschutz-Abwägung dazu steht in `doku/datenmodell/08-warenwirtschaft.md`.
  >
  > **Was NICHT überholt ist:** die Regel gilt für ALLES ANDERE unverändert weiter — kein Framework, kein Bauschritt, keine weitere Abhängigkeit für Donut-Charts, Tabellen, Dialoge oder irgendetwas, das sich mit eigenem SVG/Canvas ebenso gut zeichnen lässt. Eine Landkarte ist der eine Fall, in dem "selbst zeichnen" die eigentliche fachliche Frage verfehlt; das rechtfertigt keine zweite, dritte, vierte Ausnahme nach demselben Muster.
